...
public class login3Activity extends AppCompatActivity {
    private EditText zipCodeSpaces, phoneNumberSpaces, addressSpaces, addressDetailSpaces, birthdayDateSpaces, genderSelectSpaces;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        zipCodeSpaces = findViewById(R.id.zipCodeSpaces);
        phoneNumberSpaces = findViewById(R.id.phoneNumberSpaces);
        addressSpaces = findViewById(R.id.addressSpaces);
        addressDetailSpaces = findViewById(R.id.addressDetailSpaces);
        birthdayDateSpaces = findViewById(R.id.birthdayDateSpaces);
        genderSelectSpaces = findViewById(R.id.genderSelectSpaces);
    ...
    private void addTextWatchers() {
        ...
        zipCodeSpaces.addTextChangedListener(formValidator);
        phoneNumberSpaces.addTextChangedListener(formValidator);
        addressSpaces.addTextChangedListener(formValidator);
        addressDetailSpaces.addTextChangedListener(formValidator);
        birthdayDateSpaces.addTextChangedListener(formValidator);
    }

    // Check if all fields are valid, if so, enable btnJoin with orange color
    private void checkFormValid() {
        boolean isFormValid = !zipCodeSpaces.getText().toString().isEmpty() &&
                !phoneNumberSpaces.getText().toString().isEmpty() &&
                !addressSpaces.getText().toString().isEmpty() &&
                !addressDetailSpaces.getText().toString().isEmpty() &&
                !birthdayDateSpaces.getText().toString().isEmpty() &&
                isValidPhoneNumber(phoneNumberSpaces.getText().toString()) &&
                isValidBirthday(birthdayDateSpaces.getText().toString());
    ...
    private void saveUserDetails(String userName, String userEmail, String userId) {
        // 입력된 정보 가져오기
        String userPhone = phoneNumberSpaces.getText().toString();
        String userPost = zipCodeSpaces.getText().toString();
        String userAddress = addressSpaces.getText().toString();
        String userAddressDetail = addressDetailSpaces.getText().toString();
        String userBirthday = birthdayDateSpaces.getText().toString();
        String genderStr = genderSelectSpaces.getText().toString();
