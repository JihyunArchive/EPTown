...
public class login3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btnJoinTwo = findViewById(R.id.btnJoinTwo);  // 활성화된 회원가입 버튼

        // genderSelectSpaces 입력란에 텍스트가 변경될 때마다 버튼의 가시성 변경
        genderSelectInput.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {}
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length() > 0) {
                    // 텍스트가 있으면 기본 버튼 숨기고, 활성화된 버튼 보이기
                    btnJoin.setVisibility(Button.GONE);
                    btnJoinTwo.setVisibility(Button.VISIBLE);
                } else {
                    // 텍스트가 없으면 기본 버튼 보이게 하고, 활성화된 버튼 숨기기
                    btnJoin.setVisibility(Button.VISIBLE);
                    btnJoinTwo.setVisibility(Button.GONE);
                }
            }
